...
cp1 = axes[0].contourf(lats, plevs, mixing_ratio_grid.T, levels=20, cmap='viridis')
axes[0].invert_yaxis()  
axes[0].set_ylabel("pressure [Pa]")
axes[0].set_yscale('log')
axes[0].set_xlabel("latitude [deg]")
fig.colorbar(cp1, ax=axes[0], label="$\\mathregular{O_3}$ $\\mathregular{[\\frac{mol}{mol}]}$")

cp2 = axes[1].contourf(lats, plevs, conc_grid.T, levels=20, cmap='plasma')
axes[1].invert_yaxis()
axes[1].set_yscale('log')
axes[1].set_xlabel("latitude [deg]")
fig.colorbar(cp2, ax=axes[1], label="$\\mathregular{O_3}$ $\\mathregular{[\\frac{molec}{cm^3}]}$")
